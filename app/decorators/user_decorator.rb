class UserDecorator < Draper::Decorator
  ...
  def display_image_html options={width: '100%', height: '100%'}
    (%{<div class="avatar_wrapper" style="width: #{options[:width]}; height: #{options[:height]}">} +
  ...
  def display_credits
    number_to_currency source.credits, unit: 'US$', precision: 0, delimiter: ','
  end

  def display_total_of_backs
    number_to_currency source.backs.confirmed.sum(:value), unit: 'US$', precision: 0, delimiter: ','
  end
end
